...
import { Purchase, Category, Product } from '../types';
import { supabase } from '../lib/supabase';
import { purchaseToRow, productToRow, slugify } from '../lib/mappers';
...
export default function PurchaseForm({ categories, existing, onClose, onSaved }: Props) {
  ...
  // لو رقم السريال اتكتب واتطابق مع منتج موجود بالفعل، نجيب بياناته ونملى بيها الفورم تلقائيًا.
  useEffect(() => {
    if (lookupTimer.current) clearTimeout(lookupTimer.current);
    const value = serialNumber.trim();
    if (!value) { setSerialLookupNote(''); return; }
    lookupTimer.current = setTimeout(async () => {
      const { data } = await supabase.from('products').select('*').eq('serial_number', value).maybeSingle();
      if (!data) { setSerialLookupNote(''); return; }
      const p = data as any;
      ...
      setExtraImages((p.images ?? []).filter((i: string) => i !== p.image));
      setSerialLookupNote('الجهاز ده موجود بالفعل عندك — البيانات اتملت تلقائيًا من المنتج المسجل، وأي تعديل هنا هيحدّث بيانات المنتج.');
    }, 500);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const productId = matchedProductId ?? existing?.productId ?? `${slugify(name)}-${slugify(serialNumber || Date.now().toString(36))}`;
    const specs = { screen, processor, camera, battery };
    const colorRows = colors.filter(c => c.name.trim()).map(c => ({ name: c.name.trim(), hex: c.hex, bgClass: `bg-[${c.hex}]` }));
    ...
    const purchase: Purchase = {
      ...
      images: [mainImage, ...extraImages],
      specs,
      colors: colorRows.map(c => ({ name: c.name, hex: c.hex })),
    ...
    // بيسجل/يحدّث الجهاز كمنتج فعلي يظهر في المتجر...
    const { error: productError } = await supabase.from('products').upsert(productToRow(product));
    if (productError) {
      setSaving(false);
    ...
    // ...وبيسجل عملية الشراء نفسها في السجل.
    const row = purchaseToRow(purchase);
    const { error: purchaseError } = isEdit
    ...
    <div className="fixed inset-0 bg-black/50 flex items-start justify-center overflow-y-auto z-50 p-4" dir="rtl">
      <div className="bg-white rounded-3xl w-full max-w-2xl my-8 shadow-2xl">
        <div className="flex items-center justify-between p-6 border-b border-slate-100">
          <h2 className="text-xl font-black text-slate-900">{isEdit ? 'تعديل مشترى' : 'تسجيل مشترى جديد'}</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div className="border border-slate-100 rounded-2xl p-4 bg-slate-50/60">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <div className="flex-1 space-y-2">
                ...
                {serialLookupNote && <p className="text-[11px] text-emerald-600 font-bold">{serialLookupNote}</p>}
              </div>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={saving || uploading} className="flex-1 py-3 rounded-xl bg-[#c09d53] hover:bg-[#a9863f] text-white font-black disabled:opacity-60">
              {saving ? 'جاري الحفظ...' : isEdit ? 'حفظ التعديلات' : 'تسجيل المشترى'}
            </button>
          </div>
        </form>
      </div>
